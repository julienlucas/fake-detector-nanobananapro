...
DATA_DIR = os.path.join(_script_dir, "../static/photos")

# Choisir le modèle : définir l'un des deux (l'autre à None)
ONNX_MODEL_PATH = "../models/best_86.7_mobilenetV3_fake_detector_int8.onnx"
MODEL_PATH = None
...
def main():
    ...
    if flat_ds is not None:
        ...
        for path, pred, conf in zip(flat_ds, all_preds.tolist(), all_confs.tolist()):
            pct = conf * 100
            print(f"  {os.path.basename(path)} -> {class_names[pred]} ({pct:.1f}% confiance)")
        avg_conf = all_confs.mean().item() * 100
        print(f"\nRésumé: {n_fake} fake, {n_real} real | Confiance moyenne: {avg_conf:.1f}%")
        return
